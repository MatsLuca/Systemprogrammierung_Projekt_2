...
void test_ai_step_includes_score(void) {
    game_state_t game = physics_create_game(100,20,DIFFICULTY_EASY);
    game.bot.x = 0; game.bot.width = 10; game.score = 5;
    game.ball.x = 20;
    ai_update(&game);
    int expected = (DIFFICULTY_EASY + BOT_BASE_SPEED) + 5;
...
void test_ai_difficulty_medium_speed(void) {
    game_state_t game = physics_create_game(100,20,DIFFICULTY_MEDIUM);
    game.bot.x = 0; game.bot.width = 10; game.score = 0;
    game.ball.x = 20;
    ai_update(&game);
    TEST_ASSERT_EQUAL_INT(DIFFICULTY_MEDIUM + BOT_BASE_SPEED, game.bot.x);
}
...
void test_ai_difficulty_hard_speed_and_boundary(void) {
    game_state_t game = physics_create_game(50,10,DIFFICULTY_HARD);
